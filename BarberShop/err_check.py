    ...
    while True:
        ...
        if num == "phone":
            if answer.isdigit() and len(answer) == 11:
                break      
        if num == "birth year" or "experience since":
            if answer.isdigit() and len(answer) == 4 and 1900 < int(answer) < 2023:
                break
        answer = input(f"Data is incorrect!\n"
                       f"Use only use only the letters"
                       f" of the alphabet, the length"
                       f" of the number is 11 digits\n"
                       f" of the year and experience is 4 digits and real year\n"
                       f"Enter a {num}: ")
    return answer
